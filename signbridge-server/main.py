...
# ─────────────────────────────────────────────────────────────────────
# Signaling rooms  { room_code: { "host": WebSocket, "guest": WebSocket } }
# ─────────────────────────────────────────────────────────────────────
...
def load_model_now():
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print(f"✗ {_model_error}"); return
    size = os.path.getsize(MODEL_PATH)
    print(f"[model] {size:,} bytes")
    if size < 500_000:
        _model_error = f"File too small ({size} bytes) — Git LFS pointer?"
        print(f"✗ {_model_error}"); return
    try:
        print("[model] Importing TensorFlow…")
        import tensorflow as tf
        print(f"[model] TF {tf.__version__}")
        _model = tf.keras.models.load_model(MODEL_PATH)
        _model.predict(np.zeros((1, SEQUENCE_LEN, FEATURES), dtype=np.float32), verbose=0)
        print("✓ Model ready")
    except MemoryError:
        _model_error = "OOM — upgrade Render plan to ≥ 1 GB RAM"
        print(f"✗ {_model_error}")
    except Exception as e:
        _model_error = str(e)
        print("✗ Model load failed:"); traceback.print_exc()
...
# ─────────────────────────────────────────────────────────────────────
# WebRTC Signaling  /ws/signal/{room}/{role}
#
# How it works:
#   1. Host opens /ws/signal/SB-1234/host
#   2. Guest opens /ws/signal/SB-1234/guest
#   3. Server forwards every message from one peer to the other.
#      Messages are plain JSON WebRTC signals:
#        { type: "offer",  sdp: "..." }
#        { type: "answer", sdp: "..." }
#        { type: "ice",    candidate: {...} }
#   4. WebRTC handles the actual video — the server only relays signals.
# ─────────────────────────────────────────────────────────────────────
...
async def signaling(ws: WebSocket, room: str, role: str):
    ...
    await ws.accept()
    print(f"[signal] {role} joined room {room}")
    ...
    # Tell the guest the host is ready (so it can start the offer)
    if role == "guest" and other in rooms[room]:
        try:
            await ws.send_text(json.dumps({"type": "ready"}))
        except Exception:
            pass

    if role == "host" and other in rooms[room]:
        # Tell guest that host re-joined
        try:
            await rooms[room][other].send_text(json.dumps({"type": "ready"}))
        except Exception:
            pass
    ...
    try:
        while True:
            ...
            # Forward to the other peer if connected
            other_ws = rooms.get(room, {}).get(other)
            if other_ws:
                try:
        ...
        print(f"[signal] {role} left room {room}")
    finally:
        if room in rooms:
...
# ─────────────────────────────────────────────────────────────────────
# Sign detection WebSocket  /ws/signs
# ─────────────────────────────────────────────────────────────────────
...
def correct_probs(probs, wrist_info):
    if wrist_info is None: return probs
    c  = probs.copy()
    ...
    if -0.35 <= wy < 0.2:
        i_f = SIGN_TO_IDX.get("finish", -1); i_p = SIGN_TO_IDX.get("please", -1)
...
def make_predictor():
    ...
    def accept(word, conf):
        ...
        last_word = word; cooldown = COOLDOWN; vote_buf.clear()
    ...
    def predict(pose, left_hand, right_hand, context=""):
        ...
        if not has_hand:
            ...
            if hand_loss_count < HAND_LOSS_TOLERANCE:
                ...
                return {"word": None, "status": "hand_lost", "buffer": len(frame_buf), "needed": SEQUENCE_LEN}
        ...
        if len(frame_buf) < SEQUENCE_LEN:
            return {"word": None, "status": "buffering", "buffer": len(frame_buf), "needed": SEQUENCE_LEN}
        if _model is None:
            return {"word": None, "status": "no_model", "error": _model_error or "Model not loaded"}
        ...
        word = top_sign[0]; conf = top_conf[0]; gap = top_conf[0] - top_conf[1]
        if conf < CONF_THRESHOLD:
            vote_buf.append(None)
            return {"word": None, "top_word": word, "confidence": round(conf, 3), "status": "low_confidence"}
        if gap < CONF_GAP:
            vote_buf.append(None)
            return {"word": None, "top_word": word, "confidence": round(conf, 3), "status": "ambiguous", "gap": round(gap, 3)}
        vote_buf.append(word)
        votes = list(vote_buf); top_votes = sum(1 for v in votes if v == word); ratio = top_votes / len(votes)
        if ratio < VOTE_RATIO:
            return {"word": None, "top_word": word, "confidence": round(conf, 3), "status": "pending", "votes": f"{top_votes}/{len(votes)}"}
...
async def websocket_signs(ws: WebSocket):
    ...
    try:
        ...
        print("✗ Signs client disconnected")
    except Exception as e:
...
async def suggest_sentence(request: Request):
    ...
    try:
        ...
        if not sign_word:
            return {"sentence": "", "error": "No sign_word"}
...
Write ONE short natural reply (3-8 words) that directly answers what they said and matches the sign intent. Output only the reply, no labels.
...
Other: "Are you hungry?" | Sign: "I am hungry" → Yes, I'm really hungry."""
        resp = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Write short conversational replies for a sign language user. Output only the reply."},
